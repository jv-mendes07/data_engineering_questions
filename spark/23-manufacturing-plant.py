...
def etl(products, sales):
  ...
  w = W.partitionBy('category').orderBy(F.col('revenue').desc())
  
  revenue_rank = sales.join(products, how = 'inner', on = 'product_id')

  revenue_rank = revenue_rank.withColumn(
    'rank',
    F.dense_rank().over(w)
  ).filter(F.col('rank') <= 3)

  revenue_rank = revenue_rank.select(
    F.col('category'),
    F.col('product_name'),
    F.col('revenue'),
    F.col('rank')
