...
def etl(input_df):
  ...
  input_df = (
    input_df.withColumn('month', F.date_trunc('month', 'request_date')) \
            .groupBy('month') \
            .agg(
  ...
  input_df = input_df.withColumn('actual_distance_per_dollar', F.col('total_distance') / F.col('total_cost'))

  windowFunction = W.orderBy('month')

  input_df = input_df.withColumn('forecasted_distance_per_dollar', F.lag('actual_distance_per_dollar').over(windowFunction))

  windowFunction = W.partitionBy('month').orderBy('month')

  input_df = input_df.withColumn('error',
          F.mean(F.sqrt(F.pow(F.col('actual_distance_per_dollar') - F.col('forecasted_distance_per_dollar'), 2))).over(windowFunction))
