...
def etl(video_stream_df):
    ...
    video_stream_df = video_stream_df[video_stream_df['view_count'] >= 1000000]

    video_stream_df['years_since_release'] = current_year -video_stream_df['release_year']

    video_stream_df = video_stream_df[video_stream_df['years_since_release'] <= 6]

    video_stream_df = video_stream_df[['duration', 'genre', 'release_year', 'title', 'video_id', 'view_count']].sort_values(by = 'duration')
