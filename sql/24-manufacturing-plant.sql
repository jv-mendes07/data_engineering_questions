...
-- Write query here
-- Table name manufacture_product & manufacture_sales
...
SELECT 
  ...
  DENSE_RANK() OVER (PARTITION BY mp.category ORDER BY SUM(ms.revenue) DESC) AS rank
FROM manufacture_sales AS ms 
INNER JOIN manufacture_product AS mp
