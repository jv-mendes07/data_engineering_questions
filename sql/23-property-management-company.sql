...
-- Write query here
-- TABLE NAME: `prop_landlords` & `prop_properties`

SELECT 
  pl.landlord_id,
  CONCAT(pl.first_name, ' ', pl.last_name) AS landlord_name,
  SUM(pp.rent) AS total_rental_income 
FROM prop_landlords AS pl 
INNER JOIN prop_properties AS pp 
ON pl.landlord_id = pp.landlord_id
GROUP BY 
  pl.landlord_id,
  CONCAT(pl.first_name, ' ', pl.last_name)
ORDER BY pl.landlord_id ASC